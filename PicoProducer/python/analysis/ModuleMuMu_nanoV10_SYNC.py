...
class ModuleMuMu_nanoV10_SYNC(ModuleTauPair):
  
  def __init__(self, fname, **kwargs):
    kwargs['channel'] = 'mumu'
    super(ModuleMuMu_nanoV10_SYNC,self).__init__(fname,**kwargs)
  ...
  def beginJob(self):
    ...
    super(ModuleMuMu_nanoV10_SYNC,self).beginJob()
  ...
  def analyze(self, event):
    ...
    if self.trigger.fired(event):
       self.out.cutflow.fill('trig') 
    
    ##### MUON #######################################
    muons = [ ]
    for muon in Collection(event,'Muon'):
      if muon.pt< 10 : continue # lower pt cut
    ...
    for i, muon1 in enumerate(muons,1):
      for muon2 in muons[i:]:
        ...
        if muon1.pt< 25 and muon2.pt<25: continue # larger pt cut
        if muon1.pt > muon2.pt and abs(muon1.eta) > 2.1: continue
        if muon2.pt > muon1.pt and abs(muon2.eta) > 2.1: continue
    ...
    if self.trigger.match(event,muon1): 
    ...
    self.out.triggerfired[0]               = self.trigger.fired(event) 
    self.out.leadtrig[0]                   = self.trigger.match(event,muon1)   
    ...
    if self.ismc:
      ...
      self.out.weightTOT[0]     = self.out.puweight[0] * self.out.trigweight[0] * self.out.idisoweight_1[0] * self.out.idisoweight_2[0] * self.out.zptweight[0]
