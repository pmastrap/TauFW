...
def plotCorrelation(channel,var,region,year,*parameters,**kwargs):
    ...
    poi         = kwargs.get('poi',       ""                )
    era         = "%s-13TeV"%year
    channel_red  = channel.replace("_v10_2p5","")
    filename    = '%s/higgsCombine.%s_%s-%s%s-%s.MultiDimFit.mH90.root'%(indir,channel_red,var,region,tag,era)
...
def plotPostFitValues(channel,var,region,year,*parameters,**kwargs):
    ...
    channel_red  = channel.replace("_v10_2p5","")
    filename    = '%s/higgsCombine.%s_%s-%s%s-%s.MultiDimFit.mH90.root'%(indir,channel_red,var,region,tag,era)
    filenamesFD = '%s/fitDiagnostics.%s_%s-%s%s-%s_TES*p*.root'%(indir,channel_red,var,region,tag,era)
    ensureDirectory(outdir)
    if not name:
...
def main(args):
    ...
    compare   = {
      "norm":
        [ "xsec_tt", "xsec_st",
          "xsec_vv", "norm_qcd","rate_jTauFake_DM0_pt3","tid_SF_DM0_pt3"
        ],
      "shapes":
        [ "shape_mTauFakeSF", "shape_jTauFake_DM0_pt3", "shape_mTauFake_DM0_pt3",
          "shape_dy"
        ],
        "track":
        [ "trackedParam_xsec_dy", "trackedParam_sf_W_DM0", "trackedParam_shape_dy","trackedParam_tid_SF_DM0_pt3" 
        ],
      # "tid":
    ...
    fulllist  = [
      "xsec_tt", "xsec_st","xsec_vv", "norm_qcd","rate_jTauFake_DM0_pt3",
      "shape_mTauFakeSF", "shape_jTauFake_DM0_pt3", "shape_mTauFake_DM0_pt3","shape_dy",
      #"trackedParam_xsec_dy", 
      "trackedParam_sf_W_DM0", #"trackedParam_shape_dy","trackedParam_tid_SF_DM0_pt3"
    ...
    for var in setup["observables"]:
        ...
        for r in setup["observables"][var]["scanRegions"]:
            ...
            print(">>>>list of variables to correlate")
