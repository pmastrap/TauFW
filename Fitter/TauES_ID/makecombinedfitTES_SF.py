...
def combinedfit(setup, setup_mumu, option, **kwargs):
    ...
    npts_fit     = kwargs.get('npts_fit',     "--points=50"                                                                                                  )
    ...
    for v in setup["observables"]:
        ...
        for r in variable["scanRegions"]:
            ...
            # Fit of tid_SF_DM by DM with tes as a nuisance parameter
                ...
                tid_SF_range = "0.7,1.2" 
                if "DM0_pt3" in BINLABEL:
		    tid_SF_range = "0.85,1.2"	 
                POI = "tid_SF_%s" % (r)
                NP = "rgx{.*tid.*}" 
                print(">>>>>>> tid_"+r+" fit")
    ...
    if option == '2' or option == '4' :
      ...
      print("Executing ..")
      print("./TauES_ID/plotPostFitScan_POI.py --poi tid_SF -y %s -e %s -r %s,%s -c %s" %(era,extratag,min(tid_SF_range),max(tid_SF_range), config))
