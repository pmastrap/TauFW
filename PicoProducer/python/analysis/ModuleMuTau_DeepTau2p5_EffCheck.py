...
class ModuleMuTau_DeepTau2p5_EffCheck(ModuleTauPair):
  
  def __init__(self, fname, **kwargs):
    kwargs['channel'] = 'mutau'
    super(ModuleMuTau_DeepTau2p5_EffCheck,self).__init__(fname,**kwargs)
    ...
    #REAL TAUS, JET, E, MUON
    app = ['R','J', 'E', 'M']

    for source in app:

	    self.out.cutflow.addcut('tau_'+source,                        "tau_"+source                                )
	    self.out.cutflow.addcut('tau_idVSjet_L_2p5_'+source,          "tau_idVSjet_L_2p5_"+source                  )
	    self.out.cutflow.addcut('tau_idVSmu_L_2p5_'+source,           "tau_idVSmu_L_2p5_"+source                   )
	    self.out.cutflow.addcut('tau_idVSe_L_2p5_'+source,            "tau_idVSe_L_2p5_"+source                    )

	    self.out.cutflow.addcut('tau_idVSjet_L_2p1_'+source,          "tau_idVSjet_L_2p1_"+source                  )
	    self.out.cutflow.addcut('tau_idVSmu_L_2p1_'+source,           "tau_idVSmu_L_2p1_"+source                   )
	    self.out.cutflow.addcut('tau_idVSe_L_2p1_'+source,            "tau_idVSe_L_2p1_"+source                    )

	    self.out.cutflow.addcut('tau_idVSjet_M_2p5_'+source,          "tau_idVSjet_M_2p5_"+source                  )
	    self.out.cutflow.addcut('tau_idVSmu_M_2p5_'+source,           "tau_idVSmu_M_2p5_"+source                   )
	    self.out.cutflow.addcut('tau_idVSe_M_2p5_'+source,            "tau_idVSe_M_2p5_"+source                    )

	    self.out.cutflow.addcut('tau_idVSjet_M_2p1_'+source,          "tau_idVSjet_M_2p1_"+source                  )
	    self.out.cutflow.addcut('tau_idVSmu_M_2p1_'+source,           "tau_idVSmu_M_2p1_"+source                   )
	    self.out.cutflow.addcut('tau_idVSe_M_2p1_'+source,            "tau_idVSe_M_2p1_"+source                    )

	    self.out.cutflow.addcut('tau_idVSjet_T_2p5_'+source,          "tau_idVSjet_T_2p5_"+source                  )
	    self.out.cutflow.addcut('tau_idVSmu_T_2p5_'+source,           "tau_idVSmu_T_2p5_"+source                   )
	    self.out.cutflow.addcut('tau_idVSe_T_2p5_'+source,            "tau_idVSe_T_2p5_"+source                    )

	    self.out.cutflow.addcut('tau_idVSjet_T_2p1_'+source,          "tau_idVSjet_T_2p1_"+source                  )
	    self.out.cutflow.addcut('tau_idVSmu_T_2p1_'+source,           "tau_idVSmu_T_2p1_"+source                   )
	    self.out.cutflow.addcut('tau_idVSe_T_2p1_'+source,            "tau_idVSe_T_2p1_"+source                    )
  ...
  def beginJob(self):
    ...
    super(ModuleMuTau_DeepTau2p5_EffCheck,self).beginJob()
  ...
  def analyze(self, event):
    ...
    for tau in Collection(event,'Tau'):
      ...
      if tau.genPartFlav == 5:
         self.out.cutflow.fill('tau_R')
         source = 'R'
      #MUON FAKING TAU
      elif tau.genPartFlav == 2 or tau.genPartFlav == 4:
         self.out.cutflow.fill('tau_M')
         source = 'M'
      #ELECTRON FAKING TAU
      elif tau.genPartFlav == 1 or tau.genPartFlav == 3:
         self.out.cutflow.fill('tau_E')
         source = 'E'
      #JET FAKING TAU
      elif tau.genPartFlav == 0:
         self.out.cutflow.fill('tau_J')
         source = 'J'
      else: continue
      ...
      #2p1
      if tau.idDeepTau2017v2p1VSe>=1:  # VVVLoose
         self.out.cutflow.fill('tau_idVSe_L_2p1_'+source)
	    
      if tau.idDeepTau2017v2p1VSmu>=1:  # VLoose
         self.out.cutflow.fill('tau_idVSmu_L_2p1_'+source)
	     
      if tau.idDeepTau2017v2p1VSjet>=1: #VVVLoose
         self.out.cutflow.fill('tau_idVSjet_L_2p1_'+source)

      #2p5
      if tau.idDeepTau2018v2p5VSe>=1:  # VVVLoose
         self.out.cutflow.fill('tau_idVSe_L_2p5_'+source)

      if tau.idDeepTau2018v2p5VSmu>=1:  # VLoose
         self.out.cutflow.fill('tau_idVSmu_L_2p5_'+source)

      if tau.idDeepTau2018v2p5VSjet>=1: #VVVLoose
         self.out.cutflow.fill('tau_idVSjet_L_2p5_'+source)

	     
      #medium

      #2p1
      if tau.idDeepTau2017v2p1VSe>=5:  # M
         self.out.cutflow.fill('tau_idVSe_M_2p1_'+source)

      if tau.idDeepTau2017v2p1VSmu>=3:  # M
         self.out.cutflow.fill('tau_idVSmu_M_2p1_'+source)

      if tau.idDeepTau2017v2p1VSjet>=5: # M
         self.out.cutflow.fill('tau_idVSjet_M_2p1_'+source)

      #2p5
      if tau.idDeepTau2018v2p5VSe>=5:  # M
         self.out.cutflow.fill('tau_idVSe_M_2p5_'+source)

      if tau.idDeepTau2018v2p5VSmu>=3:  # M
         self.out.cutflow.fill('tau_idVSmu_M_2p5_'+source)

      if tau.idDeepTau2018v2p5VSjet>=5: # M
         self.out.cutflow.fill('tau_idVSjet_M_2p5_'+source)


      #tightest

      #2p1
      if tau.idDeepTau2017v2p1VSe>=8:  # VVT
         self.out.cutflow.fill('tau_idVSe_T_2p1_'+source)

      if tau.idDeepTau2017v2p1VSmu>=4 :  # T
         self.out.cutflow.fill('tau_idVSmu_T_2p1_'+source)

      if tau.idDeepTau2017v2p1VSjet>=8: # VVT
         self.out.cutflow.fill('tau_idVSjet_T_2p1_'+source)

      #2p5
      if tau.idDeepTau2018v2p5VSe>=8:  # VVT
         self.out.cutflow.fill('tau_idVSe_T_2p5_'+source)

      if tau.idDeepTau2018v2p5VSmu>=4:  # T
         self.out.cutflow.fill('tau_idVSmu_T_2p5_'+source)

      if tau.idDeepTau2018v2p5VSjet>=8: # VVT
         self.out.cutflow.fill('tau_idVSjet_T_2p5_'+source)

    if len(taus)==0: return False
    # sort by pt
    taus.sort(reverse=True, key=lambda x: x.pt)
    tau = taus[0]
