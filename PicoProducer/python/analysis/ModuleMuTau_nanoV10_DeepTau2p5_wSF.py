...
class ModuleMuTau_nanoV10_DeepTau2p5_wSF(ModuleTauPair):
  
  def __init__(self, fname, **kwargs):
    kwargs['channel'] = 'mutau'
    super(ModuleMuTau_nanoV10_DeepTau2p5_wSF,self).__init__(fname,**kwargs)
    ...
    print(">>>> Tau SF importing ...")
    #print(">>>> Info :")
    #Use Id SFs stored in TauID_SF_DeepTau2018v2p5VSjet_UL2018.json --> modify TauIDSFTool and add json flagged True to specify if root/json?
    #datapath = os.path.join(os.environ.get('CMSSW_BASE',""),"src/TauPOG/TauIDSFs/data")
    #jsonTauIdSF = os.path.join(path,"TauID_SF_DeepTau2018v2p5VSjet_%s.json"%(self.year)) 
    #ceval = correctionlib.CorrectionSet.from_file(jsonTauIdSF)
    #list(ceval.keys())
    #for corr in ceval.values():
    #    print("Correction "+ corr.name+" has "+len(corr.inputs)+" inputs")
    #for ix in corr.inputs:
    #    print("   Input "+ ix.name + " " +ix.type + " : " + ix.description)

    self.tauSFsM    = TauIDSFTool(tauSFVersion[self.year],'DeepTau2018v2p5VSjet','Medium', pTdm=True)
  ...
  def beginJob(self):
    ...
    super(ModuleMuTau_nanoV10_DeepTau2p5_wSF,self).beginJob()
  ...
  def analyze(self, event):
    ...
    if self.ismc:
      ...
      #  self.out.idweight_2[0]        = self.tauSFsT.getSFvsPT(tau.pt)
         #new function for SF binned in pT and divided also in DMs
         self.out.idweight_medium_2[0] = self.tauSFsM.getSFvsPTDM(tau.decayMode, tau.pt, tau.genPartFlav)
      ...
      self.out.weight[0]              = self.out.genweight[0]*self.out.puweight[0]*self.out.trigweight[0]*self.out.idisoweight_1[0] *self.out.idweight_medium_2[0]
    elif self.isembed:
