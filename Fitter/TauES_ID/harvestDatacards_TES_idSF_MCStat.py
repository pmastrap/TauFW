...
def harvest(setup, year, obs, **kwargs):
    ...
    if "fitRegions" in setup["observables"][obs]:
      for region in setup["observables"][obs]["fitRegions"]:
        ...
        #harvester.ExtractData("ztt", "data_obs")  # Extract the RooDataHist
        ...
        for region, DM in cats:
          ...
          newfilename_temp = datacardtxt.replace('$TAG',outdir).replace('$ANALYSIS',analysis).replace('$CHANNEL',channel).replace('$BINID',DM).replace('$ERA',era)
          newfilename = newfilename_temp.replace("v10_2p5_","")
